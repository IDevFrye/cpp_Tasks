...
int main() {
    ...
    if ((x1 == x2) || (y1 == y2)) { // критерий возможности перемещения ладьи
        std::cout << "Истина. Ладья способна переместиться заданным образом и побить фигуру, стоящую там, завершение программы." << std::endl;
        return 0;
    } 
    else {
        std::cout << "Ложь. Ладья не способна переместиться заданным образом, завершение программы." << std::endl;
        return 0;
    }
}
